...
export function generateOutline(intake) {
    return {
        scope: [
...
export function generateAgilePlan(intake) {
    const epics = [
        {
            id: "EP-1",
    ...
    const tasks = taskTemplates.map(([ownerAgent, title], index) => {
    ...
    const assignments = AGENTS.map((agentName) => ({
        agentName,
...
export function generateDeliverables(intake, project) {
